...
import {CrudState, ICrudAction, ICrudActionPayload, IStoreState, IWithId} from "../../../custom-typings/custom-typings"
...
function byIdReducer(state: CrudState = byIdInitialState, action: ICrudAction) {
  const id = action.meta ? action.meta.id : undefined
  switch (action.type) {
    case FETCH_SUCCESS:
      const data = state.toJS()
      const payload: IWithId[] = ('data' in action.payload) ? action.payload.data : action.payload
    ...
    case CREATE_SUCCESS:
      return state
      // TODO ks: check lifecycle
      // const cid = action.payload.id
      // return state.set((action.payload.id as any).toString(), fromJS({
    ...
    case GARBAGE_COLLECT:
      ...
      return state.filter((record: any, _id) => (
 ...
 */
function collectionReducer(state = collectionInitialState, action: ICrudAction) {
  switch (action.type) {
    case FETCH:
      return state.set('params', fromJS(action.meta.params))
                  ...
                  .set('error', null)
    case FETCH_SUCCESS:
      const originalPayload: ICrudActionPayload = action.payload || {}
      const payload: IWithId[] = ('data' in originalPayload) ? action.payload.data : action.payload
...
function collectionsReducer(state: Immutable.List<any> = collectionsInitialState, action: ICrudAction) {
  switch (action.type) {
    case FETCH:
    case FETCH_SUCCESS:
    case FETCH_ERROR:
    ...
    case GARBAGE_COLLECT:
      const tenMinutesAgo = action.meta.now - 10 * 60 * 1000
      return state.filter((collection: any) => (
...
function actionStatusReducer(state = actionStatusInitialState, action: ICrudAction) {
  switch (action.type) {
...
export default function crudReducer(state: CrudState = initialState, action: ICrudAction) {
  const id = action.meta ? action.meta.id : undefined
  switch (action.type) {
    ...
    case GARBAGE_COLLECT:
      return state.map((model: any) => (
               model.update('collections',
                            (s: CrudState) => collectionsReducer(s, action))
                    .update('byId',
                            (s: CrudState) => byIdReducer(s, action))
             ))
    case FETCH:
    case FETCH_SUCCESS:
    case FETCH_ERROR:
    ...
    case CREATE:
      return state.updateIn([action.meta.model, 'actionStatus'],
                            (s: CrudState) => actionStatusReducer(s, action))
    case CREATE_SUCCESS:
      return state.updateIn([action.meta.model, 'byId'],
                            (s: CrudState) => byIdReducer(s, action))
